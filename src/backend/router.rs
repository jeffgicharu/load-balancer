...
use crate::backend::algorithms::{IpHash, LeastConnections, LoadBalancer, RoundRobin, Weighted};
...
impl BackendRouter {
    ...
    pub fn new(backends: &[BackendConfig], frontends: &[FrontendConfig]) -> Self {
        ...
        for backend in backends {
            let servers: Vec<SocketAddr> = backend.servers.iter().map(|s| s.address).collect();
    ...
    pub fn get_servers(&self, backend_name: &str) -> Option<&[SocketAddr]> {
        self.backends.get(backend_name).map(|b| b.servers.as_slice())
    }
